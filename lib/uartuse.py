...
def Receive_Anl(data_buf,num,ctr):
    ...
    if data_buf[2]==0xA0:
        ...
        print("模式码更改为：",ctr.work_mode)

    elif data_buf[2] == 0xB0:  # 坐标数据功能字
        # 提取x数据（假设大端格式，2字节）
        x = (data_buf[4] << 8) | data_buf[5]
        y = (data_buf[6] << 8) | data_buf[7]
        flag = data_buf[9]
        color = data_buf[11]
        outlook = data_buf[13]
        ctr.x_now = x
        ctr.y_now = y
        ctr.found_flag = flag
        print("目标坐标：({},{})\n标志位：{}".format(ctr.x_now,ctr.y_now,ctr.found_flag))
...
def uart_data_prase(R, buf, ctr):  # 处理接受数据
    ...
    if R.state==0 and buf==0xFF:#帧头1
        ...
        # print("接收到数据帧头1：",R.uart_buf)
        # 状态机实现：检测协议头0xFE
    elif R.state==1 and buf==0xFE:#帧头2
        R.state=2
        R.uart_buf.append(buf)
        #print("接收到数据帧头2：",R.uart_buf)
    # 状态机实现：读取功能字（命令标识）
    elif R.state==2 and buf<0xFF:#功能字
        R.state=3
        R.uart_buf.append(buf)
        # print("接收到功能字：",R.uart_buf)
    # 状态机实现：读取数据长度
        ...
        R.uart_buf.append(buf)
        print("接收到数据长度：",R.uart_buf)
        print("R_data_len:{}".format(buf))
    # 状态机实现：读取有效数据
    elif R.state==4 and R._data_len>0:#存储对应长度数据
        # print("R._data_len:{}".format(R._data_len))
        R._data_len=R._data_len-1
        R.uart_buf.append(buf)
        if R._data_len==0:
            R.state=5
            print("解析完毕",R.uart_buf)
    # 状态机实现：读取校验字节
    elif R.state==5:
        R.uart_buf.append(buf)
        R.state=0
